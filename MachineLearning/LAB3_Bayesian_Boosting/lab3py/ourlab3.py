...
# # print(a.reshape(1,-1).shape)->>(1,3)
# # print(a.reshape(1,-1).reshape(-1).shape)->>(3,)
...
# u = np.array((1,2,3))
# X = X - u
# # print(X)->> array([[0, 0, 0],[3, 3, 3],[6, 6, 6]])
...
def computePrior(labels, W=None):
    ...
    for jdx, c in enumerate(classes):
        ...
        print(pointers)
        prior[c] = pointers.shape[0]/X.shape[0] #eq8
        print(prior[c])
...
def mlParams(X, labels, W=None):
    ...
    for jdx, k in enumerate(classes):
        ...
        for m in range(pointers.shape[1]):
            ...
            #print("l[m]" + str(l[m]))
            #print("sumation"+str(sumatation))
            #print("mu[m]"+str(mu[k,m]))
...
plotGaussian(X, labels, mu, sigma) # TODO. Didn't take 95% confidence interval
