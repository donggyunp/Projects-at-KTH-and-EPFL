...
class Net2:
    ...
    def init_weights(self):
        return (
            np.random.normal(0,1e-4, size=(self.hid_dim,self.in_dim+1)),
...
def plot_data(config, network, line_start, line_end, convergence_epoch, port_a, port_b):
    #data = np.delete(config['data_val'], (-1), axis=0)
    data = np.delete(config['data_val'], (-1), axis=0)
    x, y = data
    plt.scatter(x, y, c=config['labels_val'], cmap='viridis')
    plt.axis('equal')
    
    interval = 0.01
    dec_bounds = network.dec_boundary(
        line_start, line_end, 
        line_start, line_end,interval
    )
    #print("before random choice:",index_vertex.shape)
    # calculate bouandary 
    #x_cord = np.linspace(line_start,line_end)
    #y_cord = network.get_boundary(x_cord)
    #print(x.shape)
    #print(y.shape)
    #index_vertex = np.random.choice(index_vertex, x.shape[0])
    #print("after random choice:",index_vertex.shape)      
    #print(index_vertex)
    #plt.plot(x[index_vertex],y[index_vertex], 'k-')
    print("DEC shape",dec_bounds.shape)
    plt.plot(dec_bounds[0,:], dec_bounds[1,:], 'g*')
    #plt.xlim([line_start, line_end])
    
    plt.colorbar()
    '''
    plt.savefig(
        'plots/mlp_{}_lr_{}_epochs_{}_hid_{}_port{}_subexp1.png'.format(
        config['update_type'], config['learn_rate'], 
        config['epochs'], network.hid_dim, port_a
    ))
    '''
    plt.show()
...
def main():
    ...
    mean_a = np.array([0,2])
    mean_b = np.array([0.,-2])
    sigma_a = np.diag([0.5,0.5])
    sigma_b = np.diag([0.5,0.5])
    N_train = int(N*(1-val_portion_a))
    N_val = int(N*val_portion_a)

    #data_train, labels_train, data_val, labels_val = data_gen_subsample(
    #    N, mean_a, mean_b, sigma_a, sigma_b, x_dim, out_dim,exp=4)
    data_train, labels_train, data_val, labels_val = data_gen_subsample(
        N, mean_a, mean_b, sigma_a, sigma_b, x_dim, out_dim,exp=4)

    #data, labels = data_gen(N_train, mean_a, mean_b, sigma_a, sigma_b, 2, 1)
    
    #sub_sample(data, labels, experiment)

    #data_val, labels_val = data_gen(N_val, mean_a, mean_b, sigma_a, sigma_b, 2, 1)
    _,N_train = data_train.shape
    _,N_val = data_val.shape
    data_train = np.concatenate((data_train, np.ones((1,N_train))), axis=0)
    data_val = np.concatenate((data_val, np.ones((1,N_val))), axis=0)
    ...
    #create plots
    plot_data(config, net2, -6, 10, convergence_epoch,val_portion_a,val_portion_b)
    plot_errs(config, msg_epoch,msg_epoch_val, net2, 'mse')
    plot_errs(config, class_errs, class_errs_val,net2, 'classif')
